...
export default function New() {
  ...
  return (
    <AdminLayout>
      <h1 className="text-3xl font-heading font-normal tracking-wider mb-6">Add Project</h1>
      <form onSubmit={handleSubmit} className="space-y-4 max-w-xl p-6 rounded-lg bg-[#111111]  border border-[#333] shadow-2xl text-white">
        <div>
          <label className="block mb-1 font-medium text-white font-semibold">Project Name</label>
        ...
        <div>
          <label className="block mb-1 font-medium text-white font-semibold">Technologies Used</label>
        ...
        <div>
          <label className="block mb-1 font-medium text-white font-semibold">Description</label>
        ...
        <div>
          <label className="block mb-1 font-medium text-white font-semibold">GitHub Link</label>
        ...
        <div>
          <label className="block mb-1 font-medium text-white font-semibold">Live Link</label>
        ...
        <div>
          <label className="block mb-2 font-medium text-white font-semibold">Project Image</label>
          
          {imageFile ? (
            <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <span className="text-sm font-medium text-blue-700 truncate">{imageFile.name}</span>
              <button type="button" onClick={() => setImageFile(null)} className="text-blue-700 hover:text-blue-900 font-heading font-normal tracking-wider px-2">✕</button>
            </div>
          ) : (
            <div className="flex items-center justify-center w-full">
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="flex flex-col items-center justify-center pt-5 pb-6 text-[#ff5a1f]/70">
                  ...
                  <p className="mb-1 text-sm"><span className="font-semibold text-blue-600">Click to upload</span> an image</p>
                  <p className="text-xs text-[#ff5a1f]/70">PNG, JPG, WEBP or GIF</p>
                </div>
        ...
        <div className="flex items-center space-x-6 mt-4">
          <div className="flex items-center space-x-2">
            ...
            <label htmlFor="featured" className="font-medium text-white font-semibold">Featured Project</label>
          </div>
          <div className="flex items-center space-x-2">
            <label htmlFor="type" className="font-medium text-white font-semibold">Project Type:</label>
            <select id="type" className="border p-2 rounded bg-[#222] text-white" value={formData.type || 'personal'} onChange={e => setFormData({ ...formData, type: e.target.value })}>
