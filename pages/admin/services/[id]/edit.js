...
export default function Edit() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`/api/portfolio/services/${id}`).then(res => res.json()).then(data => setFormData(data.data));
  }, [id]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await fetch(`/api/portfolio/services/${id}`, {
      method: 'PUT',
      ...
      <form onSubmit={handleSubmit} className="admin-card admin-form max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select className="admin-input" value={formData.icon || 'Globe'} onChange={e => setFormData({ ...formData, icon: e.target.value })}>
        ...
        <div>
          ...
          <input required className="admin-input" value={formData.tags || ''} onChange={e => setFormData({ ...formData, tags: e.target.value })} />
        </div>
        <div>
