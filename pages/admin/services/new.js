...
export default function New() {
  const [formData, setFormData] = useState({ title: '', icon: 'Globe', tags: '', description: '', order: 0 });
    ...
    <AdminLayout>
      <div className="admin-header">
        <div>
          ...
          <h1 className="admin-title">Add Service</h1>
        </div>
      </div>
      <form onSubmit={handleSubmit} className="admin-card admin-form max-w-2xl">
        <div>
          <label className="admin-label">Title</label>
          <input required className="admin-input" placeholder="e.g. Website Design" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} />
        </div>
        ...
        <div>
          ...
          <input required className="admin-input" placeholder="MERN Stack, REST APIs, Node.js" value={formData.tags} onChange={e => setFormData({ ...formData, tags: e.target.value })} />
        </div>
        <div>
          <label className="admin-label">Description</label>
          <textarea required rows="4" className="admin-input" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })}></textarea>
        </div>
