...
export default function List() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/portfolio/education/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-heading font-normal tracking-wider">Educations</h1>
        <Link href="/admin/education/new" className="px-4 py-2 rounded bg-[#ff5a1f] text-white shadow-lg hover:bg-[#e04d19] text-white hover:opacity-90 transition-opacity border-0">Add New</Link>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-100/80 border-b border-gray-300 text-black">
              <th className="p-4 font-semibold text-white font-semibold">Institution</th>
              <th className="p-4 font-semibold text-white font-semibold">Degree</th>
              <th className="p-4 font-semibold text-white font-semibold">Duration</th>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="border-b border-gray-200 hover:bg-[#111111]">
                ...
                <td className="p-4 text-sm text-gray-400">{item.degree}</td>
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan="4" className="p-8 text-center text-[#ff5a1f]/70">No items found.</td>
              </tr>
            )}
          </tbody>
        </table>
