...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const data = await prisma.industry.findUnique({ where: { id: numId } });
  ...
  if (req.method === 'PUT') {
    try {
      const data = await prisma.industry.update({ where: { id: numId }, data: req.body });
  ...
  if (req.method === 'DELETE') {
    try {
      await prisma.industry.delete({ where: { id: numId } });
