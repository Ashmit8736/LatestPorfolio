...
export function validateService(data) {
  ...
  if (!title || !tags || !description) return 'Title, tags, and description are required.';
  return null;
}
...
export function validateServiceDetail(data) {
  const { heading, paragraph1, paragraph2, checklist } = data;
  if (!heading || !paragraph1 || !paragraph2 || !checklist) {
    return 'Heading, both paragraphs, and checklist are required.';
  }
  return null;
}
