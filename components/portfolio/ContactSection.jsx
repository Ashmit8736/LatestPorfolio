        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white/5 p-8 sm:p-10 rounded-2xl border border-white/10 backdrop-blur-md shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} className="w-full bg-[#050505] text-white px-4 py-3 border border-white/10 rounded-xl focus:ring-2 focus:ring-[#ff5a1f]/50 focus:border-[#ff5a1f] outline-none transition-all" placeholder="Tech Inc." />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Mobile Number (Optional)</label>
              <input type="tel" value={formData.phone || ''} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-[#050505] text-white px-4 py-3 border border-white/10 rounded-xl focus:ring-2 focus:ring-[#ff5a1f]/50 focus:border-[#ff5a1f] outline-none transition-all" placeholder="+91 9876543210" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Message</label>
            <textarea required rows="5" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} className="w-full bg-[#050505] text-white px-4 py-3 border border-white/10 rounded-xl focus:ring-2 focus:ring-[#ff5a1f]/50 focus:border-[#ff5a1f] outline-none transition-all resize-none" placeholder="How can I help you?"></textarea>
          </div>
          {status === 'error' && <p className="text-red-400 text-sm font-medium">Something went wrong. Please try again.</p>}
          <button disabled={status === 'loading'} className="w-full bg-[#ff5a1f] text-white font-medium py-4 rounded-xl hover:bg-[#e04d19] focus:ring-4 focus:ring-[#ff5a1f]/30 transition-all disabled:opacity-50 shadow-[0_0_20px_-5px_rgba(255,90,31,0.4)]">
